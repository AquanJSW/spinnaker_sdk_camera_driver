...
void acquisition::Capture::onInit() {
    ...
    pubThread_.reset(
        new boost::thread(boost::bind(&acquisition::Capture::run, this)));
...
void acquisition::Capture::init_variables_register_to_ros() {
        ...
        if (mem >= 1000)
            ...
            ROS_FATAL_STREAM(
                ...
                << " MB)! Must be at least 1000 MB. Run: \nsudo sh -c \"echo "
                   "1000 > /sys/module/usbcore/parameters/usbfs_memory_mb\"\n "
                   "Terminating...");
            ros::shutdown();
        }
    } else {
        ROS_FATAL_STREAM(
            "Could not check USB memory on system! Terminating...");
        ros::shutdown();
    }

    // default values for the parameters are set here. Should be removed
    // eventually!!
    ...
    ROS_INFO_STREAM("spinnaker_sdk_camera_driver package version: "
                    << spinnaker_sdk_camera_driver_VERSION);
    ...
    acquisition_pub =
        nh_.advertise<spinnaker_sdk_camera_driver::SpinnakerImageNames>(
            "camera", 1000);
    ...
    if (EXTERNAL_TRIGGER_) {
        timeStamp_sub =
            nh_.subscribe("/imu/sync_trigger", 1000,
                          &acquisition::Capture::assignTimeStampCallback, this);
    ...
    // dynamic reconfigure
    dynamicReCfgServer_ = new dynamic_reconfigure::Server<
        spinnaker_sdk_camera_driver::spinnaker_camConfig>(nh_pvt_);

    dynamic_reconfigure::Server<
        spinnaker_sdk_camera_driver::spinnaker_camConfig>::CallbackType
        dynamicReCfgServerCB_t;

    dynamicReCfgServerCB_t = boost::bind(
        &acquisition::Capture::dynamicReconfigureCallback, this, _1, _2);
...
void acquisition::Capture::load_cameras() {
    ...
    for (int j = 0; j < cam_ids_.size(); j++) {
        ...
        for (int i = 0; i < numCameras_; i++) {
            ...
            acquisition::Camera cam(camList_.GetByIndex(i));
            if (!EXTERNAL_TRIGGER_) {
                cam.setGetNextImageTimeout(
                    SPINNAKER_GET_NEXT_IMAGE_TIMEOUT_); // set to finite number
                                                        // when not using
                                                        // external triggering
            }
            ...
            if (cam.get_id().compare(cam_ids_[j]) == 0) {
                ...
                camera_image_pubs.push_back(it_->advertiseCamera(
                    "camera_array/" + cam_names_[j] + "/image_raw", 1));
                // camera_info_pubs.push_back(nh_.advertise<sensor_msgs::CameraInfo>("camera_array/"+cam_names_[j]+"/camera_info",
                // 1));

                img_msgs.push_back(sensor_msgs::ImagePtr());

                sensor_msgs::CameraInfoPtr ci_msg(
                ...
                if (region_of_interest_set_ &&
                    (region_of_interest_width_ != 0 ||
                     region_of_interest_height_ != 0)) {
                ...
                if (PUBLISH_CAM_INFO_) {
                    ...
                    for (int count = 0; count < intrinsic_coeff_vec_[j].size();
                         count++) {
                    ...
                    // else if(numCameras_ == 1){
                        ...
                        ci_msg->P = {intrinsic_coeff_vec_[j][0],
                                     intrinsic_coeff_vec_[j][1],
                                     intrinsic_coeff_vec_[j][2],
                                     0,
                                     intrinsic_coeff_vec_[j][3],
                                     intrinsic_coeff_vec_[j][4],
                                     intrinsic_coeff_vec_[j][5],
                                     0,
                                     intrinsic_coeff_vec_[j][6],
                                     intrinsic_coeff_vec_[j][7],
                                     intrinsic_coeff_vec_[j][8],
                                     0};
                    }
                }
    ...
    // numCameras_ variable is used in other methods where it means size of cams
    // list.
    numCameras_ = cams.size();
    // setting PUBLISH_CAM_INFO_ to true so export to ros method can publish
    // it_.advertiseCamera msg with zero intrisics and distortion coeffs.
...
void acquisition::Capture::read_parameters() {
    ...
    if (nh_pvt_.getParam("save_path", path_)) {
        ...
        boost::filesystem::path canonicalPath = boost::filesystem::canonical(
            ".", boost::filesystem::current_path());
        path_ = canonicalPath.string();

        ROS_WARN_STREAM(
            "  Save path not provided, data will be saved to: " << path_);
    }
    ...
    ROS_ASSERT_MSG(nh_pvt_.getParam("cam_ids", cam_id_vec),
    ...
    if (nh_pvt_.getParam("external_trigger", EXTERNAL_TRIGGER_)) {
        ROS_INFO("  External trigger: %s",
                 EXTERNAL_TRIGGER_ ? "true" : "false");
    } else
        ROS_WARN("  'external_trigger' Parameter not set, using default "
                 "behavior external_trigger=%s",
                 EXTERNAL_TRIGGER_ ? "true" : "false");

#ifndef trigger_msgs_FOUND
    if (EXTERNAL_TRIGGER_)
        ROS_WARN("  Using 'external_trigger'. Trigger msgs package not found, "
                 "will use machine timestamps");
#endif

    // Unless external trigger is being used, a master cam needs to be specified
    // If the external trigger is set, all the cameras are set up as slave int
    // mcam_int;

    int mcam_int;

    if (!EXTERNAL_TRIGGER_) {
        ROS_ASSERT_MSG(nh_pvt_.getParam("master_cam", mcam_int),
                       "master_cam is required!");
    ...
    if (nh_pvt_.getParam("utstamps", MASTER_TIMESTAMP_FOR_ALL_)) {
        ...
        ROS_INFO("  Unique time stamps for each camera: %s",
                 !MASTER_TIMESTAMP_FOR_ALL_ ? "true" : "false");
    } else
        ROS_WARN("  'utstamps' Parameter not set, using default behavior "
                 "utstamps=%s",
                 !MASTER_TIMESTAMP_FOR_ALL_ ? "true" : "false");
    ...
    if (nh_pvt_.getParam("color", color_))
        ...
        ROS_WARN("  'color' Parameter not set, using default behavior color=%s",
                 color_ ? "true" : "false");

    if (nh_pvt_.getParam("flip_horizontal", flip_horizontal_vec_)) {
        ROS_ASSERT_MSG(
            num_ids == flip_horizontal_vec_.size(),
            "If flip_horizontal flags are provided, they should be the same "
            "number as cam_ids and should correspond in order!");
        for (int i = 0; i < flip_horizontal_vec_.size(); i++) {
            ROS_INFO_STREAM("  " << cam_ids_[i] << " flip_horizontal "
                                 << flip_horizontal_vec_[i]);
        }
    } else {
        ROS_WARN_STREAM("  flip_horizontal flags are not provided. default "
                        "behavior is false ");
        for (int i = 0; i < cam_ids_.size(); i++) {
            flip_horizontal_vec_.push_back(false);
            ROS_WARN_STREAM("  " << cam_ids_[i]
                                 << " flip_horizontal set to default = "
                                 << flip_horizontal_vec_[i]);
        }
    }
    ...
    if (nh_pvt_.getParam("flip_vertical", flip_vertical_vec_)) {
        ROS_ASSERT_MSG(
            num_ids == flip_vertical_vec_.size(),
            "If flip_vertical flags are provided, they should be the same "
            "number as cam_ids and should correspond in order!");
        for (int i = 0; i < flip_vertical_vec_.size(); i++) {
            ROS_INFO_STREAM("  " << cam_ids_[i] << " flip_vertical "
                                 << flip_vertical_vec_[i]);
        }
    } else {
        ROS_WARN_STREAM("  flip_vertical flags are not provided. default "
                        "behavior is false ");
        for (int i = 0; i < cam_ids_.size(); i++) {
            flip_vertical_vec_.push_back(false);
            ROS_WARN_STREAM("  " << cam_ids_[i]
                                 << " flip_vertical set to default = "
                                 << flip_vertical_vec_[i]);
        }
    }

    if (nh_pvt_.getParam("to_ros", EXPORT_TO_ROS_))
        ROS_INFO("  Exporting images to ROS: %s",
                 EXPORT_TO_ROS_ ? "true" : "false");
    else
        ROS_WARN(
    ...
    if (nh_pvt_.getParam("live", LIVE_))
        ...
        ROS_WARN("  'live' Parameter not set, using default behavior live=%s",
                 LIVE_ ? "true" : "false");

    if (nh_pvt_.getParam("live_grid", GRID_VIEW_)) {
        LIVE_ = LIVE_ || GRID_VIEW_;
        ROS_INFO("  Showing grid-style live images setting: %s",
                 GRID_VIEW_ ? "true" : "false");
    } else
        ROS_WARN("  'live_grid' Parameter not set, using default behavior "
                 "live_grid=%s",
                 GRID_VIEW_ ? "true" : "false");
    ...
    if (nh_pvt_.getParam("max_rate_save", MAX_RATE_SAVE_))
        ...
        ROS_WARN("  'max_rate_save' Parameter not set, using default behavior "
                 "max_rate_save=%s",
                 MAX_RATE_SAVE_ ? "true" : "false");

    if (nh_pvt_.getParam("time", TIME_BENCHMARK_))
        ROS_INFO("  Displaying timing details: %s",
                 TIME_BENCHMARK_ ? "true" : "false");
    else
        ROS_WARN("  'time' Parameter not set, using default behavior time=%s",
                 TIME_BENCHMARK_ ? "true" : "false");
    ...
    if (nh_pvt_.getParam("skip", skip_num_)) {
        if (skip_num_ > 0)
            ...
            skip_num_ = 20;
            ROS_WARN("  Provided 'skip' is not valid, using default behavior, "
                     "skip=%d",
                     skip_num_);
        }
    } else
        ROS_WARN("  'skip' Parameter not set, using default behavior: skip=%d",
                 skip_num_);
    ...
    if (nh_pvt_.getParam("delay", init_delay_)) {
        if (init_delay_ >= 0)
            ...
            init_delay_ = 1;
            ROS_WARN("  Provided 'delay' is not valid, using default behavior, "
                     "delay=%f",
                     init_delay_);
        }
    } else
    ...
    if (nh_pvt_.getParam("exposure_time", exposure_time_)) {
        if (exposure_time_ > 0)
            ...
            ROS_INFO("  'exposure_time'=%0.f, Setting autoexposure",
                     exposure_time_);
    } else
        ROS_WARN("  'exposure_time' Parameter not set, using default behavior: "
                 "Automatic Exposure ");
    ...
    if (nh_pvt_.getParam("gain", gain_)) {
        if (gain_ > 0) {
            ...
            ROS_INFO("  'gain' Parameter was zero or negative, using Auto gain "
                     "based on target grey value");
    } else
        ROS_WARN("  'gain' Parameter not set, using default behavior: Auto "
                 "gain based on target grey value");

    if (nh_pvt_.getParam("target_grey_value", target_grey_value_)) {
        if (target_grey_value_ > 0)
            ROS_INFO("  target_grey_value set to: %.1f", target_grey_value_);
        else
            ROS_INFO("  'target_grey_value'=%0.f, Setting "
                     "AutoExposureTargetGreyValueAuto to Continuous/ auto",
                     target_grey_value_);
    } else
        ROS_WARN("  'target_grey_value' Parameter not set, using default "
                 "behavior: AutoExposureTargetGreyValueAuto to auto");
    ...
    if (nh_pvt_.getParam("binning", binning_)) {
        if (binning_ > 0)
            ...
            binning_ = 1;
            ROS_INFO("  'binning'=%d invalid, Using defauly binning=",
                     binning_);
        }
    } else
        ROS_WARN("  'binning' Parameter not set, using default behavior: "
                 "Binning = %d",
                 binning_);
    ...
    if (nh_pvt_.getParam("soft_framerate", soft_framerate_)) {
        if (soft_framerate_ > 0) {
            ...
            ROS_INFO("  Using Software rate control, rate set to: %d",
                     soft_framerate_);
        } else
            ROS_INFO("  'soft_framerate'=%d, software rate control set to off",
                     soft_framerate_);
    } else
        ROS_WARN("  'soft_framerate' Parameter not set, using default "
                 "behavior: No Software Rate Control ");

    if (nh_pvt_.getParam("save", SAVE_))
        ROS_INFO("  Saving images set to: %d", SAVE_);
    else
        ROS_WARN("  'save' Parameter not set, using default behavior save=%d",
                 SAVE_);

    if (SAVE_ || LIVE_) {
        if (nh_pvt_.getParam("save_type", ext_)) {
            if (ext_.compare("bin") == 0)
                SAVE_BIN_ = true;
            ROS_INFO_STREAM("    save_type set as: " << ext_);
            ext_ = "." + ext_;
        } else
            ROS_WARN("    'save_type' Parameter not set, using default "
                     "behavior save=%d",
                     SAVE_);
    }
    ...
    if (SAVE_ || MAX_RATE_SAVE_) {
        if (nh_pvt_.getParam("frames", nframes_)) {
            if (nframes_ > 0) {
                ...
                ROS_INFO("    Frames will be recorded until user termination");
        } else
            ROS_WARN("    'frames' Parameter not set, using defult behavior, "
    ...
    if (nh_pvt_.hasParam("region_of_interest")) {
        ...
        if (!nh_pvt_.getParam("region_of_interest/width",
                              region_of_interest_width_)) {
            region_of_interest_set_ = false;
        }
        if (!nh_pvt_.getParam("region_of_interest/height",
                              region_of_interest_height_)) {
            region_of_interest_set_ = false;
        }
        if (!nh_pvt_.getParam("region_of_interest/x_offset",
                              region_of_interest_x_offset_)) {
            region_of_interest_set_ = false;
        }
        if (!nh_pvt_.getParam("region_of_interest/y_offset",
                              region_of_interest_y_offset_)) {
            region_of_interest_set_ = false;
        }

        if (region_of_interest_set_) {
            ROS_INFO("  Region of Interest set to width: %d\theight: "
                     "%d\toffset_x: %d offset_y: %d",
                     region_of_interest_width_, region_of_interest_height_,
                     region_of_interest_x_offset_,
                     region_of_interest_y_offset_);
        } else
            ROS_ERROR("  'region_of_interest' Parameter found but not "
                      "configured correctly, NOT BEING USED");
    } else
        ROS_INFO_STREAM("  'region of interest' not set using full resolution");
    ...
    if (nh_pvt_.getParam("intrinsic_coeffs", intrinsics_list)) {
        ...
        ROS_ASSERT_MSG(
            intrinsics_list.size() == num_ids,
            "If intrinsic_coeffs are provided, they should be the same number "
            "as cam_ids and should correspond in order!");
        for (int i = 0; i < intrinsics_list.size(); i++) {
            std::vector<double> intrinsics;
            String intrinsics_str = "";
            for (int j = 0; j < intrinsics_list[i].size(); j++) {
                ROS_ASSERT_MSG(intrinsics_list[i][j].getType() ==
                                   XmlRpc::XmlRpcValue::TypeDouble,
                               "Make sure all numbers are entered as doubles "
                               "eg. 0.0 or 1.1");
                intrinsics.push_back(
                    static_cast<double>(intrinsics_list[i][j]));
                intrinsics_str =
                    intrinsics_str + to_string(intrinsics[j]) + " ";
            }
    ...
    if (nh_pvt_.getParam("distortion_coeffs", distort_list)) {
        ...
        for (int i = 0; i < distort_list.size(); i++) {
            ...
            for (int j = 0; j < distort_list[i].size(); j++) {
                ROS_ASSERT_MSG(distort_list[i][j].getType() ==
                                   XmlRpc::XmlRpcValue::TypeDouble,
                               "Make sure all numbers are entered as doubles "
                               "eg. 0.0 or 1.1");
    ...
    if (nh_pvt_.getParam("rectification_coeffs", rect_list)) {
        ...
        ROS_ASSERT_MSG(
            rect_list.size() == num_ids,
            "If rectification_coeffs are provided, they should be the same "
            "number as cam_ids and should correspond in order!");
        for (int i = 0; i < rect_list.size(); i++) {
            std::vector<double> rect;
            String rect_str = "";
            for (int j = 0; j < rect_list[i].size(); j++) {
                ROS_ASSERT_MSG(rect_list[i][j].getType() ==
                                   XmlRpc::XmlRpcValue::TypeDouble,
                               "Make sure all numbers are entered as doubles "
                               "eg. 0.0 or 1.1");
    ...
    if (nh_pvt_.getParam("projection_coeffs", proj_list)) {
        ...
        ROS_ASSERT_MSG(
            proj_list.size() == num_ids,
            "If projection_coeffs are provided, they should be the same number "
            "as cam_ids and should correspond in order!");
        for (int i = 0; i < proj_list.size(); i++) {
            std::vector<double> proj;
            String proj_str = "";
            for (int j = 0; j < proj_list[i].size(); j++) {
                ROS_ASSERT_MSG(proj_list[i][j].getType() ==
                                   XmlRpc::XmlRpcValue::TypeDouble,
                               "Make sure all numbers are entered as doubles "
                               "eg. 0.0 or 1.1");
    ...
    if (PUBLISH_CAM_INFO_)
        ...
        ROS_WARN(
            "  Camera coeffs not provided correctly, camera info messges "
            "intrinsics and distortion coeffs will be published with zeros.");
    ...
    for (int i = numCameras_ - 1; i >= 0; i--) {
        ...
        try {
            ...
            if (!soft) {
                ...
                if (region_of_interest_set_) {
                    ...
                    if (region_of_interest_height_ != 0)
                        cams[i].setIntValue("Height",
                                            region_of_interest_height_);
                    cams[i].setIntValue("OffsetX",
                                        region_of_interest_x_offset_);
                    cams[i].setIntValue("OffsetY",
                                        region_of_interest_y_offset_);
                }
                ...
                if (gain_ > 0) { // fixed gain
                    ...
                    if (gain_ <= max_gain_allowed)
                        ...
                        ROS_WARN("Provided Gain value is higher than max "
                                 "allowed, setting gain to %f",
                                 max_gain_allowed);
                    }
                ...
                if (target_grey_value_ > 4.0) {
                    cams[i].setEnumValue("AutoExposureTargetGreyValueAuto",
                                         "Off");
                    cams[i].setFloatValue("AutoExposureTargetGreyValue",
                                          target_grey_value_);
                } else {
                    cams[i].setEnumValue("AutoExposureTargetGreyValueAuto",
                                         "Continuous");
                ...
                if (cams[i].is_master()) {
                    if (MAX_RATE_SAVE_) {
                        ...
                        cams[i].setBoolValue("AcquisitionFrameRateEnable",
                                             false);
                ...
                } else { // sets the configuration for external trigger: used
                         // for all slave cameras in master slave setup. Also in
                         // the mode when another sensor such as IMU triggers
                         // the camera
                    cams[i].setEnumValue("TriggerMode", "On");
            ...
            if (error_msg.find("Unable to set PixelFormat to BGR8") >= 0)
                ROS_WARN("Most likely cause for this error is if your camera "
                         "can't support color and your are trying to set it to "
                         "color mode");
            ros::shutdown();
        }
    }
...
void acquisition::Capture::create_cam_directories() {
    ...
    for (int i = 0; i < numCameras_; i++) {
        ...
        if (mkdir(ss.str().c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) <
            0) {
            ROS_WARN_STREAM("Failed to create directory "
                            << ss.str()
                            << "! Data will be written into pre existing "
...
void acquisition::Capture::export_to_ROS() {
    ...
    if (EXTERNAL_TRIGGER_) {
        if (latest_imu_trigger_count_ - prev_imu_trigger_count_ > 1) {
            ROS_WARN("Difference in trigger count more than 1, latest_count = "
                     "%d and prev_count = %d",
                     latest_imu_trigger_count_, prev_imu_trigger_count_);
        }

        else if (latest_imu_trigger_count_ - prev_imu_trigger_count_ == 0) {
            double wait_time_start = ros::Time::now().toSec();
            ROS_WARN("Difference in trigger count zero, latest_count = %d and "
                     "prev_count = %d",
                     latest_imu_trigger_count_, prev_imu_trigger_count_);
            ...
            ROS_INFO_STREAM("Time gap for sync messages: "
                            << ros::Time::now().toSec() - wait_time_start);
        }
    ...
    for (unsigned int i = 0; i < numCameras_; i++) {
        img_msg_header.frame_id =
            frame_id_prefix + "cam_" + to_string(i) + "_optical_frame";
        cam_info_msgs[i]->header = img_msg_header;

        if (color_)
            img_msgs[i] = cv_bridge::CvImage(img_msg_header, "bgr8", frames_[i])
                              .toImageMsg();
        else
            img_msgs[i] =
                cv_bridge::CvImage(img_msg_header, "mono8", frames_[i])
                    .toImageMsg();
...
void acquisition::Capture::get_mat_images() {
    ...
    for (int i = 0; i < numCameras_; i++) {
        ...
        else if (cams[i].get_frame_id() != frameID)
            fid_mismatch = 1;
...
void acquisition::Capture::run_soft_trig() {
    ...
    if (!VERIFY_BINNING_) {
        ...
        for (unsigned int i = 0; i < numCameras_; i++) {
            ...
            if (!region_of_interest_set_) {
                ROS_ASSERT_MSG(
                    cams[i].verifyBinning(binning_),
                    " Failed to set Binning= %d, could be either due to "
                    "Invalid binning value, try changing binning value or due "
                    "to spinnaker api bug - failing to set lower binning than "
                    "previously set value - solution: unplug usb camera and "
                    "re-plug it back and run to node with desired valid "
                    "binning",
                    binning_);
            }
            // warn if full sensor resolution is not same as calibration
            // resolution
        ...
        while (ros::ok()) {
            ...
            if (LIVE_) {
                if (GRID_VIEW_) {
                    ...
                    char title[50];
                    sprintf(title, "cam # = %d, cam ID = %s, cam name = %s",
                            CAM_, cam_ids_[CAM_].c_str(),
            ...
            if (FIXED_NUM_FRAMES_) {
                ROS_INFO_STREAM(" Recorded frames " << count << " / "
                                                    << nframes_);
                if (count > nframes_) {
                    ROS_INFO_STREAM(nframes_
                                    << " frames recorded. Terminating...");
                    cv::destroyAllWindows();
                    break;
                }
            }
            ...
            // double total_time = grab_time_ + toMat_time_ + disp_time_ +
            // save_mat_time_;
            double total_time =
                toMat_time_ + disp_time_ + save_mat_time_ + export_to_ROS_time_;
            ...
            ROS_INFO_COND(TIME_BENCHMARK_,
                          "Times (ms):- grab: %.1f, disp: %.1f, save: %.1f, "
                          "exp2ROS: %.1f",
                          toMat_time_ * 1000, disp_time_ * 1000,
                          save_mat_time_ * 1000, export_to_ROS_time_ * 1000);

            achieved_time_ = ros::Time::now().toSec();

            if (!EXTERNAL_TRIGGER_ && SOFT_FRAME_RATE_CTRL_) {
                ros_rate.sleep();
            }
        }
...
void acquisition::Capture::update_grid() {
    ...
    for (int i = 0; i < cams.size(); i++)
        frames_[i].copyTo(grid_
                              .colRange(i * frames_[i].cols,
                                        i * frames_[i].cols + frames_[i].cols)
                              .rowRange(0, grid_.rows));
}
...
void acquisition::Capture::write_queue_to_disk(queue<ImagePtr> *img_q,
    ...
    while (imageCnt < k_numImages) {
        //     ROS_DEBUG_STREAM("  Write Queue to Disk for cam: "<< cam_no <<"
        //     size = "<<img_q->size());
        ...
        ROS_DEBUG_STREAM("  Write Queue to Disk for cam: "
                         << cam_no << " size = " << img_q->size());

        if (img_q->size() > 100)
            ROS_WARN_STREAM("  Queue " << cam_no
                                       << " size is :" << img_q->size());

#ifdef trigger_msgs_FOUND
        if (abs((int)img_q->size() -
                (int)sync_message_queue_vector_.at(cam_no).size()) > 100) {
            ROS_WARN_STREAM(
                " The camera image queue size is increasing, the sync trigger "
                "messages are not coming at the desired rate");
        }
#endif
        ...
        if (EXTERNAL_TRIGGER_) {
            ...
            ROS_INFO("time Queue size for cam %d is = %d", cam_no,
...
void acquisition::Capture::init_blaze() {
    client_blaze = nh_.serviceClient<basler_blaze::ExecuteCommand>(
        "/camera/execute_command");
    srv.request.Command = "TriggerSoftware";
}

void acquisition::Capture::trigger_blaze() {
    if (!client_blaze.call(srv) || srv.response.StatusValue != 1)
        ROS_ERROR("Trigger blaze failed.");
    else
        ROS_INFO("Trigger blaze successed.");
}

void acquisition::Capture::acquire_images_to_queue(
    vector<queue<ImagePtr>> *img_qs) {
    int result = 0;
    ...
    while ((ros::Time::now().toSec() - flush_start_time) < 3.0) {
        ...
        ROS_DEBUG("Flushing time elapsed: %.3f",
                  ros::Time::now().toSec() - flush_start_time);
    }
    ...
    for (int imageCnt = 0; imageCnt < k_numImages; imageCnt++) {
        ...
        for (int i = 0; i < numCameras_; i++) {
            try {
                //  grab_frame() is a blocking call. It waits for the next image
                ...
                filename << cam_names_[i] << "_" << cam_ids_[i].c_str() << "_"
                         << todays_date_ << "_" << std::setfill('0')
                         << std::setw(6) << imageCnt << "_" << timeStamp
                         << ext_;
                ...
                ROS_DEBUG_STREAM("Queue no. "
                                 << i << " size: " << img_qs->at(i).size());
            ...
            if (i == 0) {
                ...
                ROS_DEBUG_STREAM(
                    "Rate of cam 0 write to queue: " << 1e3 / elapsed);
            }
        }
...
void acquisition::Capture::run_mt() {
    ...
    // start
    threads.create_thread(boost::bind(&Capture::acquire_images_to_queue, this,
                                      &image_queue_vector));

    // assign a new thread to write the nth image to disk acquired in a queue
    for (int i = 0; i < numCameras_; i++)
        threads.create_thread(boost::bind(&Capture::write_queue_to_disk, this,
                                          &image_queue_vector.at(i), i));
...
void acquisition::Capture::dynamicReconfigureCallback(
    spinnaker_sdk_camera_driver::spinnaker_camConfig &config, uint32_t level) {
...
#ifdef trigger_msgs_FOUND
void acquisition::Capture::assignTimeStampCallback(
    ...
    if (first_image_received) {
        for (int i = 0; i < numCameras_; i++) {
            ...
            ROS_DEBUG("Sync trigger added to cam: %d, length of queue: %d", i,
                      sync_message_queue_vector_.at(i).size());
        }
    }
